#!/usr/bin/env python3
"""
Train Linear SVM model for IndoHoaxDetector with configurable regularization.

This script trains a Linear SVM model with a specified regularization
parameter C, evaluates it using 5-fold cross-validation, and saves the model
and metrics.

Usage:
    python train_svm.py --c_value 1.0
"""
...
def train_and_evaluate(
    ...
    """Train Linear SVM model and evaluate with cross-validation."""
    ...
    print(f"[INFO] Training Linear SVM with C={c_value}")
    model = LinearSVC(
        C=c_value,
    ...
    vectorizer_path = os.path.join(output_dir, f"tfidf_vectorizer_svm_c{c_value}_mf{max_features}_ng{ngram_range[0]}-{ngram_range[1]}.pkl")
    ...
    # Save final model
    model_path = os.path.join(output_dir, f"svm_model_c{c_value}_mf{max_features}_ng{ngram_range[0]}-{ngram_range[1]}.pkl")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Linear SVM model for hoax detection with configurable regularization."
    )
    parser.add_argument(
        "--c_value",
        type=float,
        required=True,
        help="Regularization parameter C for Linear SVM"
    )
    parser.add_argument(
        "--max_features",
        type=int,
    ...
    try:
        ...
        import json
        metrics_path = os.path.join(args.output_dir, f"svm_metrics_c{args.c_value}_mf{args.max_features}_ng{args.ngram_min}-{args.ngram_max}.json")
