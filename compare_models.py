...
DEFAULT_VECTORIZER_PATH = os.path.join(SCRIPT_DIR, "tfidf_vectorizer.pkl")
DEFAULT_TEXT_COL = "text_clean"
DEFAULT_LABEL_COL = "label_encoded"

MODEL_PATHS = {
    "Logistic Regression": os.path.join(SCRIPT_DIR, "logreg_model.pkl"),
    "Linear SVM": os.path.join(SCRIPT_DIR, "svm_model.pkl"),
    "Random Forest": os.path.join(SCRIPT_DIR, "rf_model.pkl"),
    "Multinomial Naive Bayes": os.path.join(SCRIPT_DIR, "nb_model.pkl"),
    "IndoBERT": os.path.join(SCRIPT_DIR, "indobert_model"),  # Directory for transformers
}
...
def evaluate_model(
    ...
    if is_indobert:
        # For IndoBERT, need to tokenize in batches to avoid OOM
        import torch
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model.to(device)
        model.eval()

        batch_size = 32  # Adjust if still OOM
        preds = []
